...
private def mkFromFormInstanceCmd (declName : Name) : TermElabM Command := do
  ...
  let type ← `($(mkCIdent ``FromForm) $indType)
  let instName ← mkInstName ``FromForm declName
  let body ← mkFromFormBody indVal
  ...
  if (← declNames.allM isInductive) && declNames.size > 0 then
    for declName in declNames do
      if isStructure (← getEnv) declName then
        let instCmd ← liftTermElabM <| mkFromFormInstanceCmd declName
        elabCommand instCmd
    return true
  else
    return false
